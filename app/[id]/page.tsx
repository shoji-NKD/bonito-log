...
export default async function DetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      ...
      {/* Hero gradient (Signature Texture) */}
      <div
        style={{
      ...
      >
        ...
        <div
          style={{
            ...
            height: 24, background: "var(--surface)", borderRadius: "24px 24px 0 0",
          }}
          aria-hidden="true"
        />
        <h1
          style={{
            ...
            letterSpacing: "-0.03em", color: "#fff", lineHeight: 1.2, marginBottom: 6,
          }}
        >
          {record.dish}
        </h1>
        <time
          ...
          style={{ fontSize: 13, color: "rgba(255,255,255,0.70)", fontWeight: 500 }}
        >
          {dateStr}
        </time>
      </div>
      ...
      <div style={{ padding: "16px 20px 0" }}>
        ...
        {record.photo_url && (
          ...
          <img
            ...
            style={{
              ...
              boxShadow: "0 8px 40px rgba(72,95,132,0.10)",
              display: "block",
            }}
          />
        )}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          <Link href={`/${record.id}/edit`} className="btn-glass" style={{ flex: 1, textAlign: "center" }}>
            編集
          </Link>
          <DeleteButton id={record.id} dish={record.dish} />
        </div>

        {/* Detail rows — tonal, No-Divider Rule */}
        {[
          ...
          <div
            key={row!.l}
            style={{
              background: i % 2 === 0 ? "var(--surface-lowest)" : "var(--surface-low)",
              borderRadius: 18,
          ...
          >
            ...
            <div style={{ fontSize: 15, color: "var(--on-surface)", fontWeight: 500 }}>
              {row!.node}
            </div>
          </div>
        ))}
